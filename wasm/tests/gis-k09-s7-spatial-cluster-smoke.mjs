...
const scan = gis.detectSpatialClustersV01(gis.createSpatialClusterPlanV01({ planId: "exploratory", method: "exploratory-circle", radiusMeters: 5_000, minPoints: 2, maxObservations: 10 }), observations);
assert(scan.clusters.length >= 1);
assert(scan.clusters.every(({ overlapping }) => overlapping));
assert(scan.diagnostics.some(({ code }) => code === "exploratory-method"));
