...
const plan = command.resolveEpiAiGisInspectCommand("EPIAI GIS INSPECT FILE=SelectedAsset RESULT=GisInspection", variables);
assert.equal(plan.fileVariable, "SelectedAsset");
assert.equal(plan.resultName, "GisInspection");
assert.equal(plan.canonicalSource, "EPIAI GIS INSPECT FILE=SelectedAsset RESULT=GisInspection");
assert.throws(() => command.resolveEpiAiGisInspectCommand("EPIAI GIS INSPECT FILE=Missing RESULT=GisInspection", variables), /defined file variable/);
assert.throws(() => command.resolveEpiAiGisInspectCommand("EPIAI GIS INSPECT FILE=SelectedAsset RESULT=GisInspection EXTRA=x", variables), /requires exactly/);
