...
| MATCH bounded input derivation | `app/programming/classic-match-analysis.ts` | TypeScript performs schema-aware record grouping, explicit binary-domain validation, complete 1:1 pair classification, exclusion accounting, and input limits; it produces only typed aggregate counts for a future Rust/WASM kernel and does not calculate or present statistical results |
