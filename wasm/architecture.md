...
defines AST version `0.5.0`, source spans, typed expressions, and statement nodes
for `READ`, `LIST`, `FREQ`, `MEANS`, `TABLES`, `RECODE`, `DEFINE`, `ASSIGN`, `IF`, `SELECT`, and `SORT`.
...
| Typed Classic Analysis File/View/Tools/Help shell and nine-folder Command Explorer | `app/analysis/classic-analysis-menu.ts` + `app/programming/classic-command-builder.ts` | TypeScript; Read, Define, Assign, Recode, Select/Cancel Select, Sort/Cancel Sort, List, Frequencies, Tables, and Means open field-aware source-generating dialogs; DEFINE/RECODE author visible components of the bounded full program, selected execution is separately allowlisted, and all other commands remain visible gaps |
| Classic Analysis active data, variables, bounded control flow, selection, and ordering session plus line-list Output | `app/programming/classic-session.ts` + `app/programming/classic-assignment.ts` + `app/programming/classic-if.ts` + `app/programming/classic-selection.ts` + `app/programming/classic-sort.ts` + `demo/app.ts` | TypeScript; READ resolves named current-project forms and resets Standard variables; selected DEFINE/ASSIGN manages bounded typed scalar literals and UNDEFINE removes one/all Standard variables without record mutation; IF validates both branches before one Standard-variable ASSIGN; SELECT filters cumulatively; SORT replaces ordering; cancel commands independently restore membership/order; LIST/FREQ/MEANS consume the resulting session; unsupported expressions and external READ fail closed |
