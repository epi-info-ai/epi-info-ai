...
| MATCH bounded input derivation and isolation | `app/programming/classic-match-analysis.ts` + `demo/matched-worker.ts` + `demo/matched-worker-client.ts` | TypeScript performs schema-aware record grouping, explicit binary-domain validation, complete 1:1 pair classification, exclusion accounting, and input limits; only typed aggregate counts cross into the Rust/WASM kernel through a lazy Worker with timeout, cancellation, termination, and recovery |
