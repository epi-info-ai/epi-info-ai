import { validateDotDensityLayerRecipeV01, type DotDensityJoinNormalizationV01, type DotDensityLayerRecipeV01 } from "./dot-density.ts";
...
import { clipDotDensityCandidatesV01, isDotDensityPointInsideGeometryV01, type DotDensityClipGeometryV01, type DotDensityClipResultV01 } from "./dot-density-clipping.ts";
import { buildDotDensityLegendV01, type DotDensityLegendModelV01 } from "./dot-density-legend.ts";
import { joinDotDensityDataToBoundariesV01, type DotDensityBoundaryFeatureV01, type DotDensityDataRowV01, type DotDensityJoinResultV01 } from "./dot-density-join.ts";
...
export function buildDotDensityPipelineV01(recipe: DotDensityLayerRecipeV01, input: DotDensityPipelineInputV01): DotDensityPipelineResultV01 {
  ...
  const join = joinDotDensityDataToBoundariesV01(input.boundaries, input.rows, input.boundaryKeyField, input.dataKeyField, recipe.joinNormalization);
  ...
  const placement = generateDotDensityCandidatesV01(preview.values, input.bounds, recipe, {
    contains: (featureIndex, x, y) => {
      const geometry = input.geometries.get(featureIndex);
      return geometry ? isDotDensityPointInsideGeometryV01(x, y, geometry) : false;
    },
  });
