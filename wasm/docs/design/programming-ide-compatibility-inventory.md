...
|---|---|---|---|
| LEGACY-PROGRAM-001 | Open Classic Analysis and retain the command tree, Program Editor, and Output relationship | The four-area shell, nested Program Editor menu/toolbar, and Output toolbar are visible; panes are responsive sections rather than dockable desktop windows | Preserve and test the relationship on desktop/mobile; evaluate optional resizing only after workflow parity. |
| LEGACY-PROGRAM-002 | New, open, save, save-as, print, project-backed programs, and external `.pgm7` files | Program Editor has guarded New/Open/Save/Save As/Delete, the legacy Author/Comments/Created/Updated metadata, project-backed source, bounded 1 MB `.pgm7` text import/export, and source-only browser printing; a separate Page Setup API and remote extras sync remain open | Extend sync/package conflict policy without weakening source preservation; evaluate experienced-user print fidelity and browser-specific print settings. |
| LEGACY-PROGRAM-003 | Direct text editing, selection, undo/redo, find, find-next, replace, and replace-all | CodeMirror supplies direct editing plus explicit familiar Undo, Redo, Select All, Program Beginning/End, Find, Find Next, Replace, and Replace All operations; clipboard menu actions remain visible gaps | Add permission-aware clipboard handling and expand search options only where legacy/user testing requires them. |
...
| LEGACY-PROGRAM-005 | Run the full PGM or selected commands and append results to Output | The bounded full-program path runs DEFINE/RECODE/FREQ. One selected FREQ or MEANS statement now dispatches the existing typed operation; TABLES validates fields then requires explicit value-classification review. Every other selection fails closed and is recorded | Expand selected/full execution, TABLES semantics, cancellation, structured output, and deterministic session transitions command by command. |
