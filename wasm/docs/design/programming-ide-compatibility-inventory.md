...
|---|---|---|---|
| LEGACY-PROGRAM-001 | Open Classic Analysis and retain the command tree, Program Editor, and Output relationship | Bounded V0.1 Program Editor and structured Output are visible inside Classic Analysis; complete command tree/workspace parity remains open | Reproduce the complete familiar three-part workspace responsively before introducing alternate layouts. |
...
| LEGACY-PROGRAM-003 | Direct text editing, selection, undo/redo, find, find-next, replace, and replace-all | CodeMirror supplies direct editing and undo/redo in the bounded Program Editor; familiar find/replace controls and program files remain open | Preserve familiar edit commands and keyboard paths with accessible browser controls. |
...
| LEGACY-PROGRAM-015 | Sample programs and training exercises | Official Sample PGM and course references are preserved; the IDE now offers three selectable foodborne examples that execute through the bounded AST plan | **New branch:** expand to first-class project-backed program tests using reviewed fixtures, expected outputs/tolerances, native/WASM parity, and regression reports. |
