...
| Statistics | 8 | 7 | LIST, FREQ, MEANS, TABLES, SUMMARIZE, GRAPH, and the bounded 1:1 MATCH revival have typed selected execution; broader MATCH syntax remains preserved but blocked pending expansion and field-user workflow review; TABLES renders categorical M×N counts, row/column percentages, totals, expected counts, and Pearson results without inferring exposed/case values, and true stratified binary tables append Rust/WASM Mantel–Haenszel and homogeneity output from their displayed orientation; GRAPH renders bounded horizontal Bar, vertical Column, and category-share Pie branches |
| Advanced Statistics | 7 | 7 | Complex Sample Frequencies, Complex Sample Tables, and Complex Sample Means have bounded typed dialogs and browser execution. Conditional `LOGISTIC` has a source-first bounded candidate for simple terms with one `MATCHVAR`; ordinary logistic regression and survival remain visible gaps. |
...
| `MEANS` | AST 0.7 | V0.1 ordinary + V0.1 complex sample | Executes one numeric field against the active session; `PSUVAR` routes to bounded Complex Sample Means with one optional cross-tabulation domain, design stratum, and numeric weight. Browser-adapted `OUTTABLE` materializes visible result rows for session READ/LIST; the desktop CSM control is disabled and defines no result schema | Mechanical foodborne Complex Sample Means command-tour component plus adapted OUTTABLE/READ/LIST fixture | Ordinary GROUPVAR expansion, ordinary cross-tabulation and tests/ANOVA, multiple survey fields/domains, persistent output adapters, meaningful survey corpus, desktop differential output, Rust migration |
...
| `MATCH` | AST 1.0, all five retained grammar forms | Revival V0.1 authors exposure, outcome, and one match variable; broader retained options remain visible but blocked | Explicit unweighted single-`MATCHVAR` 1:1 execution uses typed pair derivation and the V0.16 Rust/WASM Worker; Output, common history, multi-command tour, cancellation, and aggregate-only field-review JSON export are active | Bounded method contract, hand-auditable CSV/expected fixture, realistic workbook, real-editor `.pgm7`, JupyterLite comparison, browser acceptance, and fingerprinted review-evidence schema | Collect experienced-user evidence and desktop comparisons where available; add zero-discordance and invalid-boundary program files; define variable-ratio/multiple-match-field/weight/OUTTABLE behavior before expanding execution; the inspected desktop executor itself is unimplemented |
| `LOGISTIC` | AST 1.0 preserves outcome, terms, `MATCHVAR`, `WEIGHTVAR`, `TITLETEXT`, `PVALUE`, `OUTTABLE`, `LINKFUNCTION`, and `NOINTERCEPT` | No source dialog yet | Conditional V0.1 accepts simple numeric/binary predictors with one `MATCHVAR`, exactly one case and at least one control per included set, and renders coefficients, adjusted odds ratios, Wald limits, likelihood-ratio fit, exclusions, and candidate warnings | Final step of the matched case-control command tour; paired analytical identity is asserted in Phase 0 | Ordinary logistic regression, categorical expansion, interactions, weights, OUTTABLE, Rust/WASM migration, independent 1:m reference comparison, legacy output and error wording, desktop differential validation |
