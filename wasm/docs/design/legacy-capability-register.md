...
| REG-0040 | LEGACY-PROGRAM-004, LEGACY-PROGRAM-005, LEGACY-PROGRAM-015 | Adapted branch: selectable runnable example programs | Adds three foodborne examples for life-stage groups by Sex, broad age bands by Case Status, and age decades. Selection loads ordinary visible/editable source with declared required fields; every example passes the same AST, semantic checks, bounded planner, history, and output path as user source | Foodborne fixture and Program Editor browser test / 2026-08-28 | Phase 5B example library V0.1; project-backed examples and broader curriculum promotion remain open |
