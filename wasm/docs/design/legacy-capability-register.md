...
| REG-0047 | LEGACY-PROGRAM-001, LEGACY-PROGRAM-002, LEGACY-PROGRAM-003, LEGACY-PROGRAM-005, LEGACY-MENU-005, LEGACY-MENU-008 | Adapted branch: Program Editor and Output command surfaces | Preserves the shipped Program Editor File/Edit/Fonts menus and toolbar plus the Output navigation toolbar in exact source order. Safe editor navigation, undo/redo/select-all, bounded Run Commands, Output navigation, and History are active; persistence, clipboard, search, print, bookmark, maximize, clear, and cancellation remain named gaps | `ProgramEditor.Designer.cs/.resx`, `OutputWindow.Designer.cs`, User Guide Classic Analysis workspace, and browser regression test / 2026-08-31 | Structural/behavioral parity candidate; saved PGM and remaining commands open |
...
| REG-0054 | LEGACY-PROGRAM-004, LEGACY-PROGRAM-005, LEGACY-PROGRAM-009, LEGACY-MENU-005, LEGACY-MENU-008 | Adapted branch: SELECT/CANCEL SELECT Classic session semantics | Adds familiar Select and Cancel Select Command Explorer paths, field-aware source dialogs, a schema-aware fail-closed plan for one field-to-literal comparison, and cumulative `AND` filtering that follows `Rule_Select`. The session reports total/selected/excluded/missing counts; LIST/FREQ/MEANS consume selected records; bare SELECT and CANCEL SELECT restore the READ source. Unsupported compound expressions/functions remain visible source but cannot run | `SelectDialog`, `CancelSelectDialog`, `Rule_Select`, AST 0.3, typed selection tests, and foodborne fixture / 2026-09-01 | Phase 5B candidate; complete expression/function and missing syntax, legacy collation/differential testing, full-program sequencing, Output wording, and user review remain open |
