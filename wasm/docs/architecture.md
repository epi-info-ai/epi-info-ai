...
## Dataset independence and example binding

Classic Analysis commands are general language operations, not behaviors coded
for a particular demonstration dataset. A command resolves identifiers against
the active project schema, consumes the active record set, and returns a
versioned result independent of file name, project name, or example catalog.
Command acceptance requires a hand-auditable fixture plus at least one realistic
project dataset where practical; additional epidemiologic domains are regression
cases for the same command contract, not separate implementations.

Dataset-specific `.pgm`/`.pgm7` files remain useful teaching and validation
artifacts. Their fingerprinted catalogs only decide whether the Program Editor
offers an example and whether its required fields are compatible. Catalogs must
never change parser, planner, execution, or statistical semantics. This keeps a
foodborne, vaccine-preventable-disease, matched case-control, or future project
free to use the same command set whenever its schema satisfies the command.
...
| MATCH paired odds ratio, conditional exact interval, McNemar statistics, exact p, and mid-p from oriented discordant counts | `matched_*` | Rust/WASM V0.16 bounded revival candidate; independent notebook agreement is required and desktop parity remains open |
...
| PMTiles v3 validation, browser-local map-asset storage, bounded raster/vector rendering, portable backup, and recovery | `app/maps/pmtiles-import.ts` + `app/maps/pmtiles-reader.ts` + `app/maps/maplibre-pmtiles.ts` + `app/maps/project-map-assets.ts` + `app/contracts/project-archive.ts` + `app/forms/study-area-picker.ts` + `demo/maps.ts` + `app/contracts/core.ts` | TypeScript new branch; explicit `File` grant, bounded validation, commit-on-Apply OPFS writes, SHA-256 provenance, and cleanup. PMTiles raster/vector packages retain their dedicated runtime integrity and recovery path. Uploaded GeoJSON and GeoTIFF layers are also stored under digest-addressed OPFS paths with display configuration in the project snapshot. Save Project As emits a bounded binary `.epia` envelope with the V2 manifest followed by raw deduplicated PMTiles, GeoJSON, and GeoTIFF payloads; Open Project validates every payload before restoration. Missing/corrupt/unavailable assets fail visibly. Field-offline acceptance remains a separate gate |
...
| MATCH bounded input derivation, execution, and isolation | `app/programming/classic-match-analysis.ts` + `demo/matched-worker.ts` + `demo/matched-worker-client.ts` + `demo/app.ts` | TypeScript performs schema-aware record grouping, explicit binary-domain validation, complete 1:1 pair classification, exclusion accounting, and input limits; only typed aggregate counts cross into the Rust/WASM kernel through a lazy Worker with timeout, cancellation, termination, and recovery. Program Editor selected/full-program paths render visible Output and record common history; sequential runs retain a read-only snapshot, and broader retained grammar remains fail-closed |
...
- [x] Add a distinct complete-project encrypted-package V0.2 path. Top-level
  File commands assemble the validated `.epia` working copy first, including
  all forms/records, saved programs, code tables, study-area metadata, audit
  history, and attached PMTiles, GeoJSON, and GeoTIFF map assets, and then protect the entire archive as
  `.epiax`. Opening decrypts and validates into a non-mutating inventory before
  the user explicitly replaces the current project. A dedicated automated
  runbook carries this boundary through optional Secure Epi Info Share handoff.
  Other attachment and future plugin payload classes remain a declared gap.
