...
| `EPIAI RECORDLINK ... RESULT=name` plus `REVIEW`, `CLUSTER`, `AUDIT`, and `OUTPUT ... TO="file.duckdb"` stages | Explainable, reviewed patient-record linkage and deduplication across two sources | Typed V0.4 base plan and V0.11 staged workflow execute bounded classification, pause for explicit review, build conflict-aware clusters, prepare fingerprint-bound audit tables, display person output, and—after acknowledgement—create a DuckDB analytical file containing person, provenance, audit, and manifest tables | Executable governed-DuckDB candidate V0.11; no stage downloads automatically, while project-table creation, source mutation, and `MERGE` remain fail-closed |

### Open increments within implemented new branches

- RECORDLINK project-table creation and reviewed
